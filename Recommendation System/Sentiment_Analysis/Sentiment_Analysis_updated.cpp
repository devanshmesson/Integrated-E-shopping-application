...
vector<pair<int,string>>colab_data;
int cnt=0;
...
ifstream fiin;
fiin.open("sentiment_integrate.txt");
int cn=0;
while(1)
{
  int phoneid;
  ...
  if(phoneid==-1)break;
  getline(fiin,line);
  colab_data.push_back({phoneid,line});
...
int max_senti_score=INT_MIN,recommended_phone;
int phone_score[10000]={0};
for(int i=0;i<cn;i++)
{
  ...
  int id_phone=colab_data[i].first;

  if(polarity==1)
  {
    phone_score[id_phone]++;
    if(max_senti_score<phone_score[id_phone])
    {
      max_senti_score=phone_score[id_phone];
      recommended_phone=id_phone;
    }
...
cout<<phone_score[7451]<<endl;
cout<<phone_score[2875]<<endl;
cout<<phone_score[7464]<<endl;
  ...
  cout<<"Recommended phone having id is:"<<recommended_phone<<endl;
//----------------------------------------------------------------------------------
}
